...
Vim = (function() {
  function Vim() {
    ...
    this.skipRegex = __bind(this.skipRegex, this);
    this.cutLine = __bind(this.cutLine, this);
    this["delete"] = __bind(this["delete"], this);
    this.newLine = __bind(this.newLine, this);
    this.insert = __bind(this.insert, this);
    this.gotoStart = __bind(this.gotoStart, this);
    this.prevWord = __bind(this.prevWord, this);
    this.gotoEnd = __bind(this.gotoEnd, this);
    ...
    Object.defineProperties(this, {
      ...
      '$cursorContainer': {
        get: function() {
          return document.querySelector('.cursor-container');
        }
      },
      'x': {
        get: function() {
          return self.cursorPosition.value()[0];
        },
        set: function(x) {
          return self.cursorPosition.value([x, this.y]);
        }
      },
      'y': {
        get: function() {
          return self.cursorPosition.value()[1];
        },
        set: function(y) {
          if (this.lines.length - 1 >= y) {
            return self.cursorPosition.value([this.x, y]);
          }
        }
      },
      'currentLine': {
        get: function() {
          return self.lines[self.cursorPosition.value()[1]];
        }
      },
      'lines': {
        get: function() {
          return self._lines.value().split('\n');
        },
        set: function(val) {
          return self._lines.value(val.join('\n'));
        }
      },
      'mode': {
        get: function() {
      ...
      'searchString': {
        ...
        set: function(val) {
          this.$search.textContent = val;
          return this._searchString.value(val);
        }
      }
    });
    this.$inputBuffer = document.getElementById('input-buffer');
    this._mode = trx.createProperty('visual');
    this._searchString = trx.createProperty('');
    this._yankRegister = trx.createProperty([]);
    this._lines = trx.createProperty(this.$editor.textContent);
    this.isCursorIdle = trx.createProperty();
    this.isCursorIdle.value(true);
    this.inputEvents = trx.fromDomEvent('input', this.$inputBuffer).map(function(e) {
      e.char = e.target.value[e.target.value.length - 1];
      e.target.value = '';
      return e;
    });
    this.keydownEvents = trx.fromDomEvent('keydown', this.$inputBuffer).map(function(e) {
      return keys['' + e.keyCode];
    }).truethy();
    trx.fromDomEvent('blur', this.$inputBuffer).subscribe(function(e) {
  ...
  Vim.prototype.render = function(x, y) {
    ...
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      l = _ref[i];
      if (this.y === i) {
        cursorContainerHtml.push(l.substr(0, x) + this.$cursor.outerHTML + l.substr(x));
      } else {
        cursorContainerHtml.push(l);
      }
    }
    return this.$cursorContainer.innerHTML = cursorContainerHtml.join('\n');
  };

  Vim.prototype.gotoEnd = function() {
    return this.x = this.currentLine.length - 1;
  };

  Vim.prototype.prevWord = function() {
    return this.skipRegex(/[^\s\.]+(?:\s+|\.)/gi, true);
  };

  Vim.prototype.gotoStart = function() {
    return this.x = 0;
  };
  ...
  Vim.prototype.newLine = function() {
    var lines;
    lines = this.lines;
    this.y++;
    lines.splice(this.y, 0, '');
    this.lines = lines;
    return this.render();
  };

  Vim.prototype["delete"] = function() {
    var lines;
    lines = this.lines;
    console.log(lines[this.y] + '}');
    lines[this.y] = lines[this.y].substr(0, this.x - 1) + lines[this.y].substr(this.x);
    console.log(lines[this.y] + '}');
    this.lines = lines;
    this.x--;
    return this.render();
  ...
  Vim.prototype.cutLine = function() {
    var lines;
    lines = this.lines;
    lines.splice(this.y, 1);
  ...
  Vim.prototype.skipRegex = function(regex, reverse) {
    var line, result, results, y;
    y = this.y;
    if (!reverse) {
      line = this.lines[y].slice(this.x);
      ...
      return this.x += (results != null ? results[0].length : void 0) || 0;
    } else {
      line = this.lines[y].slice(0, this.x);
      results = line.match(regex);
      if (!results) {
        y--;
        line = this.lines[y];
      ...
      result = (results != null ? results[results.length - 1].length : void 0) || 0;
      return this.x -= result;
    }
  };

  Vim.prototype.nextWord = function() {
    this.skipRegex(/[^\s\.]*[\s\.]+/gi);
    return void 0;
  };

  Vim.prototype.init = function() {
    var down, left, right, self, ticker, tickerEvents, up;
    self = this;
    tickerEvents = trx.createStream();
    ticker = setInterval(function() {
      return tickerEvents.publish({
        type: 'tick',
        timeStamp: new Date
      });
    }, 300);
    this.inputEvents.merge(tickerEvents).createHistory(2).filter(function(events) {
      return events.length > 1;
    }).map(function(events) {
      return events[1].timeStamp - events[0].timeStamp;
    }).createProperty(function(isIdle, time) {
      if (time > 300) {
        return isIdle = true;
      } else {
        return isIdle = false;
      }
    }).subscribe(function(isIdle) {
      if (isIdle) {
        return self.$cursor.classList.add('idle');
      } else {
        return self.$cursor.classList.remove('idle');
      }
    });
    this.inputHistory = this.inputEvents.createHistory();
    this._mode.subscribe(function(modeName) {
      return self.$body.className = modeName;
    });
    right = this.inputEvents.filter(function() {
      return self.mode === 'visual';
    }).filter(function(e) {
      return e.char === 'l';
    }).map([1, 0]);
    left = this.inputEvents.filter(function() {
      return self.mode === 'visual';
    }).filter(function(e) {
      return e.char === 'h';
    }).map([-1, 0]);
    down = this.inputEvents.filter(function() {
      return self.mode === 'visual';
    }).filter(function(e) {
      return e.char === 'j';
    }).map([0, 1]);
    up = this.inputEvents.filter(function() {
      return self.mode === 'visual';
    }).filter(function(e) {
      return e.char === 'k';
    }).map([0, -1]);
    ...
    this.inputEvents.filter(function() {
      ...
      if (e.char && e.char.length > 0) {
        return self.insert(e.char);
      }
    });
    this.inputEvents.filter(function() {
      return self.mode === 'visual';
    ...
    this.inputHistory.filter(function(events) {
      return events.length > 1 && self.mode === 'visual' && get(events, -1).char + get(events, -2).char === 'dd';
    }).subscribe(function(e) {
      self.inputHistory.reset();
      return self.cutLine();
    });
    this.keydownEvents.filter(function(key) {
      return self.mode === 'input' && key === 'del';
    }).subscribe(function() {
    ...
    this.inputEvents.filter(function() {
      ...
      if (e.char) {
        return self.searchString += e.char;
      }
    });
    this._searchString.subscribe(function(string) {
      return self.render();
    });
    this.cursorPosition = left.merge([right, up, down]).createProperty(function(position, vector) {
    ...
    this.cursorPosition.subscribe(function(position) {
      return self.render(position[0], position[1]);
    });
    return this.render();
  };
